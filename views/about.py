...
st.markdown(
    f"""
    Across most of my program office work, the pattern I saw most
    consistently wasn't about how work got reported — it was about how
    work connected to outcomes. Teams were shipping steadily. The
    delivery was real. But nobody could quite say how any of it was
    contributing to the bottom line. We were producing outputs and
    calling them progress, and the connection back to what the business
    actually needed was implicit at best.

    I ran the reporting-side work you'd expect too — consolidating
    status reports across projects into consistent templates, so a VP
    wasn't reading ten different formats a week. That work matters, and
    once it was done it stayed done. But it exposed a related problem:
    consistent format doesn't mean relevant content. What actually
    needed a VP's attention on any given Monday kept shifting — new
    customer pressure, a hiring decision, something from a board
    conversation — and my read on what would rise to the top could be
    off, because their view of the week ahead was sometimes ahead of
    mine.
    ...
    Those three problems shaped the app. Its core job is to make the
    connection between work and outcomes explicit — strategy flows into
    objectives and the key results that define them, then initiatives
    contribute to those key results, objectives, and strategy. Outputs
    and outcomes both, measured against the plan. Once that connection
    is real, the reporting-side problems become tractable: I've used AI
    along the way — to help define and measure, and to find signals in
    the data that surface risks earlier than a team would normally
    catch them on their own. There are a few different axes on which AI
    shows up in the workflow, and I've tried to be deliberate about
    which ones move first.
